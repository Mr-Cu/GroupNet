...
import torch
from data.dataloader_mssd import MSSDDataset, seq_collate
from model.GroupNet_mssd import GroupNet
...
def draw_result(future,past,mode='pre'):
    ...
    for idx in range(50):
        ...
        court = plt.imread("datasets/mssd/court.png")
        plt.imshow(court, zorder=0, extent=[Constant.X_MIN, Constant.X_MAX, Constant.Y_MAX, Constant.Y_MIN],alpha=0.5)
        if mode == 'pre':
            plt.savefig('vis/mssd/'+str(idx)+'pre.png')
        else:
            plt.savefig('vis/mssd/'+str(idx)+'gt.png')
    print('ok')
    return 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_save_dir', default='saved_models/mssd')
