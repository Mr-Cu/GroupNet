...
def draw_result(future,past,mode='pre'):
    ...
    for idx in range(50):
        ...
        court = plt.imread("datasets/traj/court.png")
        plt.imshow(court, zorder=0, extent=[Constant.X_MIN, Constant.X_MAX, Constant.Y_MAX, Constant.Y_MIN],alpha=0.5)
        if mode == 'pre':
            plt.savefig('vis/traj/'+str(idx)+'pre.png')
        else:
            plt.savefig('vis/traj/'+str(idx)+'gt.png')
    print('ok')
    return 
...
def test_model_all(test_loader, args):
    ...
    for data in test_loader:
        future_traj = np.array(data['future_traj']) * args.traj_scale # (BN,T,2)
        # print(future_traj.shape) #(139, 12, 2)
        # batch = future_traj.shape[0]
        batch = args.batch_size
        # print(batch) #139,实际应为32
        # actor_num = future_traj.shape[1]
        # y = np.reshape(future_traj,(batch*actor_num,args.future_length, 2))
        y = future_traj
        y = y[None].repeat(20,axis=0) #(BN,20,T,2)
        # print(y.shape) #(20, 139, 12, 2)
        with torch.no_grad():
            prediction = model.inference(data)
        # print(prediction.shape) #torch.Size([20, 139, 12, 2])
        prediction = prediction * args.traj_scale
        # print(prediction.shape) #torch.Size([20, 139, 12, 2])
        prediction = np.array(prediction.cpu()) #(BN,20,T,2)
        # print(prediction.shape) #(20, 139, 12, 2)
        # assert 1<0
        ...
        l2error_avg_40s += np.mean(np.min(np.mean(np.linalg.norm(y[:,:,:10,:] - prediction[:,:,:10,:], axis = 3),axis=2),axis=0))*batch
        l2error_dest_40s += np.mean(np.min(np.mean(np.linalg.norm(y[:,:,9:10,:] - prediction[:,:,9:10,:], axis = 3),axis=2),axis=0))*batch
        l2error_avg_44s += np.mean(np.min(np.mean(np.linalg.norm(y[:,:,:11,:] - prediction[:,:,:11,:], axis = 3),axis=2),axis=0))*batch
        l2error_dest_44s += np.mean(np.min(np.mean(np.linalg.norm(y[:,:,10:11,:] - prediction[:,:,10:11,:], axis = 3),axis=2),axis=0))*batch
        l2error_overall += np.mean(np.min(np.mean(np.linalg.norm(y[:,:,:12,:] - prediction[:,:,:12,:], axis = 3),axis=2),axis=0))*batch
        l2error_dest += np.mean(np.min(np.mean(np.linalg.norm(y[:,:,11:12,:] - prediction[:,:,11:12,:], axis = 3),axis=2),axis=0))*batch
        all_num += batch
    ...
    l2error_avg_40s /= all_num
    l2error_dest_40s /= all_num
    l2error_avg_44s /= all_num
    l2error_dest_44s /= all_num
    print('Dataset: {}, Pred Len: {}, ADE: {:.2f}, FDE: {:.2f}'.format(args.dataset, args.future_length, l2error_overall, l2error_dest))
...
if __name__ == '__main__':
    ...
    args.model_save_dir='saved_models/traj/'+ args.dataset
    if(args.dataset=='stanford'):
        args.delim='space'

    test_path = get_dset_path(args.dataset, 'test')
