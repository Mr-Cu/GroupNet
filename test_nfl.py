...
import torch
from data.dataloader_nfl import NFLDataset, seq_collate
from model.GroupNet_nfl import GroupNet
...
def draw_result(future,past,mode='pre'):
    ...
    for idx in range(50):
        ...
        court = plt.imread("datasets/nfl/court.png")
        plt.imshow(court, zorder=0, extent=[Constant.X_MIN, Constant.X_MAX, Constant.Y_MAX, Constant.Y_MIN],alpha=0.5)
        if mode == 'pre':
            plt.savefig('vis/nfl/'+str(idx)+'pre.png')
        else:
            plt.savefig('vis/nfl/'+str(idx)+'gt.png')
    print('ok')
    return 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_save_dir', default='saved_models/nfl')
