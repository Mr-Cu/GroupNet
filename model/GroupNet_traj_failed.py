...
class PastEncoder(nn.Module):
    ...
    def forward(self, inputs,batch_size, agent_num):
        # print(self.interaction_hyper.scale)
        self.interaction_hyper.scale=int(agent_num/batch_size)
        # print(self.interaction_hyper.scale)
        self.interaction_hyper2.scale=int(int(agent_num/batch_size)/2)
        # print(inputs.shape) #torch.Size([406, 8, 4])
        # print(inputs.shape) #torch.Size([384, 8, 4])
        
        length = inputs.shape[1]
        # print(length) #8
        
        tf_in = self.input_fc(inputs).view(agent_num, length, self.model_dim)
        # print(tf_in.shape) #torch.Size([384, 8, 64])

        tf_in_pos = self.pos_encoder(tf_in, num_a=agent_num)
        # print(tf_in_pos.shape) #torch.Size([384, 8, 64])
        tf_in_pos = tf_in_pos.view(batch_size, int(agent_num/batch_size), length, self.model_dim)
        # print(tf_in_pos.shape) #torch.Size([32, 12, 8, 64])
        ftraj_input = self.input_fc2(tf_in_pos.contiguous().view(batch_size, int(agent_num/batch_size), length*self.model_dim))
        # print(ftraj_input.shape) #torch.Size([32, 12, 64])

        query_input = F.normalize(ftraj_input,p=2,dim=2)
        feat_corr = torch.matmul(query_input,query_input.permute(0,2,1))
        # print(feat_corr.shape) #torch.Size([32, 12, 12])
        ...
        output_feature = final_feature.view(agent_num,-1)
...
class FutureEncoder(nn.Module):
    ...
    def forward(self, inputs, batch_size,agent_num,past_feature):
        ...
        tf_in_pos = self.pos_encoder(tf_in, num_a=agent_num)
        tf_in_pos = tf_in_pos.view(batch_size, int(agent_num/batch_size), length, self.model_dim)
        # print(tf_in_pos.shape) #torch.Size([407, 1, 12, 64])

        ftraj_input = self.input_fc2(tf_in_pos.contiguous().view(batch_size, int(agent_num/batch_size), -1))
        query_input = F.normalize(ftraj_input,p=2,dim=2)
        feat_corr = torch.matmul(query_input,query_input.permute(0,2,1))
        ftraj_inter,_ = self.interaction(ftraj_input)
        # print(ftraj_input.shape) #torch.Size([407, 1, 64])
        # print(feat_corr.shape) #torch.Size([407, 1, 1])
        ...
        final_feature = final_feature.view(agent_num,-1)
...
class GroupNet(nn.Module):
    ...
    def forward(self,data):
        device = self.device
        # batch_size = data['past_traj'].shape[0] #407
        batch_size = self.args.batch_size
        agent_num = data['past_traj'].shape[0]
        # agent_num = 1
        # print(batch_size) #407,相当于32个batch和对应agent_num的乘积之和
        # print(agent_num) #1
        # print(data['past_traj'].shape) #torch.Size([721, 8, 2])
        # assert 1<0
        
        past_traj = data['past_traj'].view(agent_num,self.args.past_length,2).to(device).contiguous()
        future_traj = data['future_traj'].view(agent_num,self.args.future_length,2).to(device).contiguous()
        # print(past_traj.shape) #torch.Size([721, 8, 2]),721应该是64个样本的agent数量之和
        # print(future_traj.shape) #torch.Size([721, 12, 2])
        # assert 1<0

        past_vel = past_traj[:,1:] - past_traj[:,:-1, :]
        past_vel = torch.cat([past_vel[:,[0]], past_vel], dim=1)
        # print(past_vel.shape) #torch.Size([721, 8, 2])

        future_vel = future_traj - torch.cat([past_traj[:,[-1]], future_traj[:,:-1, :]],dim=1)
        # print(future_vel.shape) #torch.Size([406, 12, 2])
        cur_location = past_traj[:,[-1]]
        # print(cur_location.shape) #torch.Size([406, 1, 2])

        inputs = torch.cat((past_traj,past_vel),dim=-1)
        inputs_for_posterior = torch.cat((future_traj,future_vel),dim=-1)

        agent_num = int(agent_num/batch_size)*batch_size

        past_traj=past_traj[0:agent_num,:,:]
        future_traj=future_traj[0:agent_num,:,:]
        past_vel=past_vel[0:agent_num,:,:]
        future_vel=future_vel[0:agent_num,:,:]
        cur_location=cur_location[0:agent_num,:,:]
        
        inputs=inputs[0:agent_num,:,:]
        # print(inputs.shape)
        inputs_for_posterior=inputs_for_posterior[0:agent_num,:,:]

        past_feature = self.past_encoder(inputs,batch_size,agent_num)
        # print(past_feature.shape)
    ...
    def inference(self, data):
        ...
        agent_num = int(agent_num/batch_size)*batch_size
        # agent_num = 1
        # print(batch_size) #139
        # print(data['past_traj'].shape) #torch.Size([139, 8, 2])

        past_traj = data['past_traj'].view(agent_num,self.args.past_length,2).to(device).contiguous()
        # print(past_traj.shape) #torch.Size([139, 8, 2])
